...
$(document).ready(function(){
	...
	for(var k = 0; k < (metadata.hash).length; k++){
		$('#BIRTable tr').last().append('<td>' + parseFloat((metadata.hash)[k][0]) + '</td>');
	}
	$('#BIRTable').append('</tr>');

	$('#BIRTable').append('<tr><td>Status</td>');
	for(var k = 0; k < (metadata.hash).length; k++){
		$('#BIRTable tr').last().append('<td>' + parseFloat((metadata.hash)[k][1]) + '</td>');
	}
	$('#BIRTable').append('</tr>');

	for(var i = 0; i < bir.length; i++){
		$('#BIRTable').append('<tr><td>' + parseFloat(bir[i].dep) + '</td>');
		for(var j = 0; j < (bir[i].ranges).length; j++){
			$('#BIRTable tr').last().append('<td>' + parseFloat((bir[i].ranges)[j]) + '</td>');
		}
		$('#BIRTable').append('</tr>');
	}
});
