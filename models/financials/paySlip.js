...
exports.get_edit = function(req, callBack){
	var db = req.db;
	var PhilHealth = db.get("PhilHealth");
	var Employees = db.get("Employees");
	var SSS = db.get("SSS");
			...
			Employees.find({employed: true}, function(err2, doc){
				...
				BIR.find({}, function(err3, bir){
					...
					metadata.findOne({"name": "BIR"},function(err4, metadata){
						...
						adviceNumbers.findOne({"name": "paySlip"}, function(err5, an){
							if(err5) return callBack(err5);
							paySlip.find({}, function(err6, paySlip){
								callBack(null, doc, ph, sss, bir, metadata, an, paySlip);
							});
						});	
					});
				});
...
//=========================== start of change =============================
	...
	return Promise.all([b,c,d,e,f]).then(values => {
		// return values;
		var hash = values[0];
		var BIRdoc = values[1];
		var PHdoc = values[2];
		var SSSdoc = values[3];
		var employee = values[4];
		var range = BIRdoc.ranges;
		var i;
		// return [currentAdviceNumber, hash, BIRdoc, PHdoc, SSSdoc, range];
		...
		console.log(data[0]);
		return paySlip.update({adviceNumber: currentAdviceNumber}, data[0]);
	}).then((doc) => {
		if(doc){
			// res.status(500).send('Insert error');
			// return(true);
...
exports.editThirteenth = function(req, res, callBack){
	...
	Employees.findOne({"eID": eID}, function(err, employee){
		...
		paySlip.update({adviceNumber: currentAdviceNumber}, {
			"eID": eID,
			"adviceNumber": currentAdviceNumber,
			"dateIssued": dateIssued,
			"issuedBy": issuedBy,
